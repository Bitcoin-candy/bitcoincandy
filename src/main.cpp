...
#define REWARD_CUT_BLOCK 20000000
...
uint256 hashGenesisBlock("0x000003156bdb3c4e3c362632b45a7d40dab75f44c345f819a3fdcbbb9187fa7c");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitCoinCandy: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitCoinCandy Signed Message:\n";
...
{
    // BitCoinCandy: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // BitCoinCandy: each block
static const int64 nTargetSpacing = 3 * 60; // BitCoinCandy: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitCoinCandy: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, BitCoinCandy - DarkGravity, written by Evan Duffield - evan@BitCoinCandy.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitCoinCandy: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BitCoinCandy: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "Bitcoin Candy X";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x33c003b7a5942fe05bb79beb5e964946925782ccfa0ed48800c3f54dc6b18da3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // BitCoinCandy: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitCoinCandyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitCoinCandyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitCoinCandyMiner(CWallet *pwallet)
{
    printf("BitCoinCandyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BitCoinCandy-miner");
    ...
    try { while (true) {
        ...
        printf("Running BitCoinCandyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitCoinCandyMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitCoinCandyMiner, pwallet));
}
